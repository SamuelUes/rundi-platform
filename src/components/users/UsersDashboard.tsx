...
export interface AdminUserRow extends CmsUserRow {
  appRole: string | null;
  phone: string | null;
  isDriver: boolean;
  hasDesignatedDriverService: boolean;
...
export function UsersDashboard({
  ...
  description =
    "Listado global de usuarios de la app, su rol, contacto y estado dentro del CMS.",
  showCards = true,
}: UsersDashboardProps) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(firebaseAuth, async (user) => {
      ...
      try {
        ...
        const appUsersById: Record<string, { email: string | null; name: string | null; appRole: string | null; phone: string | null }> = {};

        usersSnap.forEach((docSnap) => {
          const data = docSnap.data() as { email?: string; name?: string; role?: string; phone?: string };
        ...
        const cmsByUserId: Record<string, { role: string | null; assigned_service_areas: string[] }> = {};

        cmsSnap.forEach((docSnap) => {
          const data = docSnap.data() as { role?: string; assigned_service_areas?: unknown };
          const areas = Array.isArray(data.assigned_service_areas) ? (data.assigned_service_areas as string[]) : [];
        ...
        const baseRows: CmsUserRow[] = Object.entries(appUsersById).map(([id, app]) => {
        ...
        try {
          ...
          if (res.ok) {
            const body = (await res.json()) as { drivers: AdminDriverFromApi[] };
            driversByUserId = Object.fromEntries((body.drivers || []).map((d) => [d.userId, d]));
          }
        } catch {
          // silencioso
        }
        ...
        const enriched: AdminUserRow[] = baseRows.map((u) => {
          ...
          return {
            ...
            isDriver: !!driver,
            hasDesignatedDriverService: driver?.hasDesignatedDriverService ?? false,
          };
        });
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((u) => {
      ...
      const appRoleLabel = u.isDriver ? (u.hasDesignatedDriverService ? "conductor designado" : "conductor") : baseRole || "cliente";
  ...
  return (
    <section className="space-y-4">
      ...
      {loading && <p className={`${typography.body} ${themeColors.textSecondary}`}>Cargando usuarios...</p>}

      {!loading && error && <p className={`${typography.body} ${themeColors.dangerText}`}>{error}</p>}
          ...
          <table className="min-w-full border-collapse text-left text-sm">
            <thead className="bg-linear-to-r from-emerald-50 to-white text-[11px] uppercase tracking-wide text-emerald-700">
              <tr>
                ...
                {showCards && <th className="px-4 py-3 text-right font-semibold">Tarjetas</th>}
              </tr>
            </thead>
            <tbody>
              {filteredUsers.map((u, index) => {
                ...
                const cmsRoleClass = u.role ? "bg-indigo-50 text-indigo-600" : "bg-zinc-100 text-zinc-500";
                const assignedAreas = u.assigned_service_areas.length ? u.assigned_service_areas.join(", ") : "Sin áreas asignadas";

                return (
                  <tr
                    key={u.id}
                    className={`${index % 2 === 0 ? "bg-white" : "bg-zinc-50/60"} border-b last:border-0 transition hover:bg-emerald-50/60`}
                  >
                    <td className="px-4 py-3 align-top text-xs text-zinc-700">
                      <div className="space-y-0.5">
                        <div className="text-sm font-semibold text-zinc-900">{u.name || "(sin nombre)"}</div>
                        <div className="text-[11px] text-zinc-500">{u.email || "(sin correo)"}</div>
                        <div className="text-[11px] text-zinc-400">UID: {u.id}</div>
                      </div>
                    </td>
                    <td className="px-4 py-3 align-top">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${appRoleClass}`}>{appRoleLabel}</span>
                    </td>
                    <td className="px-4 py-3 align-top">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${cmsRoleClass}`}>{cmsRoleLabel}</span>
                    </td>
                    <td className="px-4 py-3 align-top text-xs text-zinc-600">
                      <div>{u.phone || "(sin teléfono)"}</div>
                    </td>
                    <td className="px-4 py-3 align-top text-xs text-zinc-600">{assignedAreas}</td>
                    {showCards && (
      ...
      {showCards && cardsModalUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-3xl rounded-lg bg-white p-5 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className={`${typography.h2} ${themeColors.textPrimary}`}>Tarjetas guardadas</h2>
            ...
            {cardsLoading && <p className={`${typography.body} ${themeColors.textSecondary}`}>Cargando tarjetas...</p>}

            {!cardsLoading && cardsError && <p className={`${typography.body} ${themeColors.dangerText}`}>{cardsError}</p>}

            {!cardsLoading && !cardsError && cards.length === 0 && (
              <p className={`${typography.body} ${themeColors.textSecondary}`}>No hay tarjetas guardadas para este usuario.</p>
            )}
                ...
                {cards.map((card) => (
                  <div key={card.id} className="rounded-lg border p-3 shadow-sm">
                    <div className="flex items-center justify-between text-sm font-semibold">
                      <span className={themeColors.textPrimary}>{card.brand?.toUpperCase() || "Tarjeta"} ·••{card.last4 || "XXXX"}</span>
                      {card.isDefault && <span className="text-xs font-medium text-emerald-600">Predeterminada</span>}
                    </div>
