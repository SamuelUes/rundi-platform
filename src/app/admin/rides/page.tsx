...
export default function AdminRidesPage() {
  ...
  const [dateFilterDraft, setDateFilterDraft] = useState({ from: "", to: "" });
  const [dateFilter, setDateFilter] = useState<{ from: string | null; to: string | null }>({
    from: null,
    to: null,
  });
  const [routeModalRide, setRouteModalRide] = useState<AdminRideRow | null>(null);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(firebaseAuth, async (user) => {
      ...
      try {
        ...
        const body = (await res.json()) as { rides: AdminRideRow[] };
        setRides(body.rides || []);
        setLoading(false);
      } catch (err) {
  ...
  async function openRouteModal(ride: AdminRideRow) {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/rides/${ride.id}/route`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const routeEmbedUrl = useMemo(() => {
    ...
    const waypoints = waypointPoints.length
      ? `&waypoints=${encodeURIComponent(
          waypointPoints.map((p) => `${p.lat},${p.lng}`).join("|")
        )}`
      : "";
  ...
  function getRideSortValue(ride: AdminRideRow, field: RideSortField) {
  ...
  return (
    <section className="space-y-5">
      <header className="space-y-1">
        <h1 className={`${typography.h1} ${themeColors.textPrimary}`}>Viajes</h1>
        <p className={`${typography.body} ${themeColors.textSecondary}`}>
          Historial consolidado de rides registrados en Supabase.
        </p>
      </header>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.1fr,0.9fr]">
          ...
          <div className="rounded-2xl border border-zinc-100 bg-white/80 p-4">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              {[
                { label: "Desde", key: "from", value: dateFilterDraft.from },
                { label: "Hasta", key: "to", value: dateFilterDraft.to },
              ].map((field) => (
                <label key={field.key} className="text-xs font-semibold uppercase tracking-wide text-zinc-500">
                  ...
                  <div className="relative mt-1">
                    ...
                    <input
                      ...
                      value={field.value}
                      onChange={(e) =>
                        setDateFilterDraft((prev) => ({ ...prev, [field.key]: e.target.value }))
                      }
          ...
          <table className="min-w-full text-left text-[13px]">
            <thead className="bg-zinc-50/70">
              <tr className="text-[11px] uppercase tracking-wide text-zinc-500">
                ...
                <th className="px-3 py-2 font-medium text-center">Mapa</th>
              </tr>
            </thead>
            <tbody>
              ...
              <div className="space-y-5">
                <div className="rounded-2xl border bg-gray-50/80 p-3 shadow-inner">
                  {routeEmbedUrl ? (
                    ...
                    <p className={`${typography.body} ${themeColors.textSecondary}`}>
                      No se pudo generar el mapa. Verifica que exista al menos un
                      origen y destino y que la clave de Google Maps sea válida.
                    </p>
                  )}
                </div>
